...
- Host-wide — Apply restrictions to all processes on the host
- Container Only — Apply restrictions only to processes running inside containers

## Operation Modes

safeguard can be run in two modes:

- **Monitor Mode** — Logs security events without blocking them. Useful for testing and validating policies before enforcement.
- **Block Mode** — Blocks security events that match the configured rules. Used in production environments.
