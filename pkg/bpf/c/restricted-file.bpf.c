...
static __noinline int is_str_match(unsigned char *map_path, unsigned char *path) {
  for (int i = 0; i < 20; i++) {//NAME_MAX
    if (map_path[i] == '\0')
      break;

    if (map_path[i] != path[i])
      return 0;
  }
...
static inline int get_file_perm(struct file_open_audit_event *event,struct file *file){
    ...
    // get_path_str_from_path(path, event);
    ...
    // bpf_printk("event->path: %s\n", file_path);
    bpf_probe_read(event->path, sizeof(event->path), file_path);
    // get_path_str_from_path(&(event->path), path, string_buf);
    bpf_printk("event->path: %s\n", event->path);

    // bpf_probe_read_kernel_str(event->path, NAME_MAX, file->f_path.dentry->d_iname);
...
// #pragma unroll
//     for(int i = 0; i < 3; i++){
//         //bpf_printk("loop-test: %d\n", i);
//         u32 index = i;
//         struct file_path *map_path = (struct file_path *)bpf_map_lookup_elem(&denied_access_files, &index);
//         if(!map_path) break;

//         // ret = is_str_match(map_path->path, event->path);
//         // if(ret < 0) {
//         //     bpf_printk("Access Denied: %s\n", event->path);
//         //     // ret = -EPERM;
//         //     break;
...
//         size_t size = strlen(map_path->path, NAME_MAX);
//         // bpf_printk("Access Size: %d\n", size);
//         int str_res = strcmp(map_path->path, event->path, size);
//         bpf_printk("Access res: %d\n", str_res);
//         // if (str_res) {
//         //     //bpf_printk("Access Denied: %s\n", event->path);
//         //     ret = -EPERM;
//         //     break;
